...
[◀ Prev: 5.0 Host Commands](5.0-host-commands.md) · [Contents](README.md) · [Next: 5.2 Templates & Conditionals ▶](5.2-templates-and-conditionals.md)

_Reconstructed edition - every claim carries an evidence tag; see [Contents](README.md) for the legend._

`$NAME$` [text variables](../../glossary.md) - sequences usable in [host commands](5.0-host-commands.md), [queries](5.5-file-transfer-and-queries.md#rip_query), button commands and keystroke macros: **data** variables substitute a value, **active** variables perform an action on the terminal. The 2.0 engine carries into 3.x intact; the 3.x inventory is known from three overlapping sources - RIPtel 3.1's help files (roughly 60 data-returning and 60 command-performing variables), the demo corpus (**269 distinct `$…$` forms** actually used in shipping scripts), and SyncTERM's **103-entry built-in table** - and where the sources disagree, the divergence is called out on the entry.

_Evidence: HLP (RIPTEL.HLP text-variable chapter); corpus; SyncTERM (ripper.c:7633-7738 sorted built-in table)._

[Text variables](#text-variables) · [Resolution in 3.x](#variable-resolution-in-3x) · [Creation & query](#text-variable-creation-and-query) · [Syntax notation](#text-variable-syntax-descriptions) · [Version & vendor](#version-number-and-vendor-text-variables) · [Date & time](#date-and-time-text-variables) · [Sound](#sound-related-text-variables-see-44) · [Mouse](#mouse-related-text-variables) · [Text windows](#text-window-related-text-variables) · [Ports/viewports](#portviewport-related-text-variables) · [Terminal operation](#terminal-operation-text-variables) · [Resets](#ripscrip-reset-related-text-variables) · [Environment configuration](#system-environment-configuration-text-variables) · [Clipboard](#clipboard-related-text-variables) · [Mouse fields](#mouse-field-related-text-variables) · [Screen](#screen-related-text-variables) · [Data tables & backup areas](#data-table-and-backup-area-text-variables) · [Image placement](#image-placement-text-variables) · [Column-text paging](#column-text-system-paging) · [File-playback prefixes](#file-playback-prefixes)
...
2. **User-defined variables** on a miss (see [below](#what-are-user-variables)).
...
| --- | --- | --- |
| `$>FILE$` `$)FILE$` `$<FILE$` `$(FILE$` | Local playback of a RIP scene, WAV, BMP, or JPEG | [5.3 Local File Playback](5.3-local-playback.md) |
...
| `$?N$` | Expand template slot N | [below](#template-slot-dereference-n) and [5.2 Templates & Conditionals](5.2-templates-and-conditionals.md) |
| `$*…$` `$+…$` `$%…$` `$#…$` | User-variable prompt directives | [below](#directive-characters-in-3x) |
...
The wire command [RIP_DEFINE](5.5-file-transfer-and-queries.md#rip_define) (`!|1D`) creates a variable. SyncTERM's handler quotes what appears to be original TeleGrafix specification prose in its comments, giving the definition-text grammar:

```text
variable-identifier[,field-width]:[?question-text?][default-value]
```

Example (from the in-source spec quote): `FULL_NAME,30:?What is your full name?John Doe`. Field width defaults to 60 when omitted. The command's numeric flags are `001` save to the [persistent variable store](../../glossary.md), `002` a blank response is not allowed, `004` non-interactive query. When a definition is interactive - or a non-interactive one finds no stored value - the terminal raises an input dialog honoring the field width and no-blank flag.
...
#### The 3.x Prompt Grammar

The RIPtel help documents the complete definition/prompt syntax usable directly in the macro language:
...
> **Editor's note:** the 2.00a4 draft (reproduced below as the inherited behavioral description) assigns the memory-table directive the character `=` and states that an ignored request inserts the value `NONE`; the RIPtel help documents `%` for the memory-table directive and the literal text `NULL` for an empty response. The 3.x record wins; the draft positions are retained below as history.

An optional `x,y:` prefix positions the prompt dialog in [world coordinates](../../glossary.md); either value may be omitted (keeping the comma) to center on that axis, and `$,:NAME$` centers on both. The placement, format-class (`ANY`/`ALPHA`/`NUMBER`/`ALPHANUM`), conversion (`TONAME`/`TOUPPER`/`TOLOWER`), width, question and default rules are as in the inherited description below, all confirmed by the help files. _(HLP; 2.00a4)_
...
The RIPtel help confirms the shipping limits: variable names are **20** characters (letters, digits, underscore; must start with a letter; auto-uppercased), question and default text are **100** characters each, and variable content (the value) is **255** characters. _(HLP)_
...
`$?N$` - where `N` is one of the 36 slot characters `0`-`9`/`A`-`Z` - expands to the stored [template](5.2-templates-and-conditionals.md) text for that slot. _(SyncTERM (ripper.c:11918-11923))_

### Host Deletion and Data Queries in 3.x

The host may delete a variable; RIPtel asks the user to confirm ("The host wants to delete the variable %s. Proceed?"). A _data query_ is simply a host-sent template of variable references - e.g. `$FULL_NAME$^m$ST_ADDR$^m$CITY$, $STATE$ $ZIP$^m` - which the terminal returns filled in; protected variables trigger a "Data Security Activated" notice (see [User Defined Variables and Data Security](#user-defined-variables-and-data-security) above). _(HLP)_
...
_Evidence: 2.00a4 - the syntax-description notation is inherited into 3.x unchanged._
...
3.x adds the bookmark-identity variables [`$USERNAME$`](#username) and [`$PASSWORD$`](#password) and the random-value variable [`$RAND$`](#rand). _Evidence: HLP (RIPTEL.HLP text-variable chapter; RIPSCRIP.HLP tvarProc string table); 2.00a4; SyncTERM (ripper.c:8055)._
...
_Evidence: 2.00a4; HLP - present in the RIPSCRIP.DLL processor table ("IFS (with LIST/category keywords)"), with the `LIST` and `_CATEGORY` directives as documented for 2.00a4._
...
_Evidence: 2.00a4; HLP; corpus (BUTTONS.RIP, MENU.RET) - heavily used by the demo scripts as the empty branch of `<<IF>>` conditionals (41 uses)._

### $PASSWORD$

_Password stored in the current bookmark_

_New in 3.x._

**Format:** `$PASSWORD$` **Syntax:** `$PASSWORD$`

Returns the password stored in the current dialing-directory bookmark ("Data Security" prompts guard host access to protected data). New in the 3.x client environment; see the [connection directory](../../glossary.md) model.
...
**Format:** `$RAND$` **Syntax:** `$RAND$`

Returns a random value. Listed in the HLP data-variable table; parameterization (range) unconfirmed _(hypothesis)_. Used by the demo menus to pick a random screen wipe.

_Evidence: HLP; corpus (TELCMDS.RET "Choose a random wipe")._
...
In 3.x the help shows the same `RIPSCRIP015300`-style format (`RIPSCRIP0` + major + minor×100, zero-padded); SyncTERM returns `RIPSCRIP015410` or `RIPSCRIP030001` depending on its emulation mode. This is distinct from the shorter `RIPSCRIP03000` auto-detect handshake reply (see [1.1 Protocol & Command Hierarchy](1.1-protocol-and-command-hierarchy.md#ansi-sequences-auto-sensing)).

_Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8055-8058)._
...
_Evidence: 2.00a4; HLP - present in the RIPSCRIP.DLL processor table (`tvarProcTERMINFO`); unknown keywords return `NONE`._

### $USERNAME$

_User-ID stored in the current bookmark_

_New in 3.x._

**Format:** `$USERNAME$` **Syntax:** `$USERNAME$`

Returns the user-ID stored in the current dialing-directory bookmark. New in the 3.x client environment; the counterpart of [`$PASSWORD$`](#password).
...
The full 1.54/2.00a4 date and time group survives intact in 3.x - all thirteen date variables and all seven time variables are in the HLP table and in SyncTERM's `rv_date`/`rv_time` handler groups. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8061 `rv_date`; 8148 `rv_time`)._
...
All seven survive in 3.x: 2.00a4 defines optional parameters for every one of them (repeat counts, frequency/duration pairs, sweep parameters); the HLP confirms the DLL validates start/stop frequency ordering and rejects zero increments/durations, and SyncTERM implements the default (parameterless) behaviors, two of them with source comments reading `// Literally in the spec.` - full detail in [4.4](4.4-tone-and-sound-generation.md).
...
SyncTERM formats coordinate fields as **4 digits in graphics mode and 2 digits in text mode** - matching 2.00a4's rule that a text-window query returns text-cell coordinates in `XX`/`YY` form while graphics queries return `XXXX`/`YYYY`. _Evidence: SyncTERM (ripper.c:8290 `rv_mouse`; 8293, 8300 field widths); 2.00a4; HLP._

### $FIELDID$

_ID number of the current mouse field_

_New in 3.x._

**Format:** `$FIELDID$` **Syntax:** `$FIELDID$`

Returns the ID number of the current mouse field - the counterpart of the `ID=n:` prefix on 3.x mouse-field [host commands](5.0-host-commands.md). Used by the demos for computed variable names (`$&MSG<<FIELDID>>$`).

_Evidence: HLP; corpus (MENU.MSE)._
...
The demo scripts switch cursors on field entry/exit - `$MCURSOR(4)$` busy, `$MCURSOR(6)$` hand, `$MCURSOR(0)$` arrow. _Evidence: 2.00a4; HLP (`tvarProcMCURSOR`); corpus (DEMO-01.COL, MENU.FN)._
...
> **Divergence:** 2.00a4 documents zero-padded _decimal_ output (`0297:0321`); SyncTERM emits **hexadecimal** (`%0*x:%0*x`). Which the shipping 3.0 driver used is unconfirmed _(hypothesis: decimal, per the documentation lineage)_. _(SyncTERM (ripper.c:8312-8320))_
...
The window-introspection group (`$TWX0$`/`$TWY0$`/`$TWX1$`/`$TWY1$`/`$TWW$`/`$TWH$`/`$TWIN$`/`$TWFONT$`) is confirmed for 3.x by SyncTERM's `rv_termstat` handler group; `$CURX$`/`$CURY$`/`$CURSOR$`/`$ISEXTWIN$` rest on the spec and help files, the management variables (`$ATW$`, `$MTW$`, `$TWGOTO$`, `$TWHOME$`, `$TWERASEEOL$`) are attested by the RIPTEL.HLP command-variable table, and the erase pair `$ETW$`/`$EGW$` by SyncTERM's `rv_erase`. The parameter grammar (window, type `BOUND`/`TEXT`/`CELL`, domain `WORLD`/`DEVICE`/`COLS`/`ROWS`) carries into 3.x unchanged as far as the evidence shows. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8734 `rv_termstat`; 8696 `rv_erase`)._
...
`$COFF$$DTW$` is the standard demo-scene prologue. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8801 group); corpus (BLUEBACK.FN)._
...
_Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8726 `rv_disable`); corpus (14 uses)._
...
_Location of the last graphical text output_

_New in 3.x._

**Format:** `$TEXTXY$` **Syntax:** `$TEXTXY$`

Returns the location of the last graphical text output (X/Y). New in the 3.0 HLP inventory; return format unconfirmed _(hypothesis: `X:Y` pair like [`$XY$`](#xy))_.
...
The five values match the five terminal text modes listed in RIPtel's MESSAGES.HLP - fonts 2 and 4 are the 91-column [MicroANSI](../techspecs/3.3-microansi-maf-delta.md) modes. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8734 group)._
...
The `$PORT…$` query family - renamed from 1.54's `$VIEW…$` forms in the late 2.00a4 drafts (see the rename note at the top of this page) - is confirmed for 3.x by the HLP data-variable inventory; the viewport-management processors are attested by their DLL names (`tvarProcAVP`, `tvarProcDVP`, `tvarProcMVP`). _Evidence: HLP (RIPTEL.HLP data-variable table; RIPSCRIP.HLP tvarProc names); 2.00a4 for the inherited parameter grammar; SyncTERM (ripper.c:8696 `rv_erase` for `$EGW$`)._
...
> **Editor's note:** the 3.x help lists `$OFFSCREEN$` with the one-liner "offscreen bitmap port pixel data", which could be read as a host-side readback of offscreen port _contents_ rather than the 2.00a4 pixel-budget query above; the wire format of any such readback is unrecovered _(hypothesis)_. _(HLP)_
...
All are action variables (expand to empty) unless noted as returning a value; the demo screens routinely hide the status bar (`$SBAROFF$`, 7 uses in the corpus). _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8801 group); corpus (DRAGON.RIP)._
...
SyncTERM carries all ten fixed-slot forms in its built-in table as stubs - the handler only prints a TODO message. _Evidence: 2.00a4; HLP (`tvarProcAPP`); SyncTERM (ripper.c:9041-9046 `rv_exploit`)._
...
`$D(1)$` is the single most-used variable in the demo corpus (197 uses, pacing the wipe effects). _Evidence: 2.00a4; HLP (`tvarProcD`); corpus (SHADOW.FN, WIPE\*.FN)._
...
_Launch the web browser on a stored URL_
...
Note that the parameter is a _variable name_, not a literal URL. See also [5.0 Host Commands](5.0-host-commands.md).
...
SyncTERM implements full `tab_next_field` highlight tracking for the Tab-navigation pair. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8974-9014, 9025-9035)._
...
> **Divergence:** SyncTERM implements `$VT102ON$`/`$VT102OFF$` as stubs (effectively no-ops); RIPtel advertises real built-in VT-102 emulation. _(2.00a4; HLP; WP; SyncTERM (ripper.c:8895-8903))_

### HLP-Attested Processors With Unrecovered Semantics
...
Bare `$RESET$` is inherited from 1.54: it performs the same operation as [RIP_RESET_WINDOWS](1.3-world-view-virtual-canvas.md#rip_reset_windows) (a "soft" reset); SyncTERM implements it as its full-reset handler. 3.x keeps the _parameterized_ reset, but the HLP documents a **flat keyword list** rather than 2.00a4's full `(option, element, sub-element)` grammar (retained in the entry below as the inherited draft surface). The DLL string table names one handler per keyword (`resetKeywordSOFT`, `resetKeywordHARD`, `resetKeywordMCURSOR`, `resetKeywordKEYBOARD`, `resetKeywordSOUND`, `resetKeywordQUERY`, `resetKeywordTV`), which suggests the keyword set below is the complete shipping surface:

| Keyword | Behavior | Evidence |
| --- | --- | --- |
| `$RESET(SOFT)$` | Soft reset - as RIP_RESET_WINDOWS | HLP; 2.00a4 |
| `$RESET(HARD)$` | Hard reset - as a [RIP_HEADER](1.1-protocol-and-command-hierarchy.md#rip_header) hard-reset | HLP; 2.00a4 |
...
| `$RESET(TV)$` | Reset text-variable state _(hypothesis: 2.00a4's `TW` text-window reset, renamed/retargeted - the DLL handler is `resetKeywordTV` and 3.x elsewhere uses `TV` to mean "text variable", cf. `$INUSE(TV,…)$`)_ | HLP |
...
| `$RESET(OVERFLOW)$` | Reset (delete) all overflow paging buffers of the [column text system](3.1-column-text-system.md); issued by NEWSPAPR.RIP before building a fresh article ("Reset all overflow files") | corpus (NEWSPAPR.RIP) |

The `OVERFLOW` keyword appears only in the corpus - it is a 3.x addition tied to the column system and is absent from both the 2.00a4 grammar and the HLP keyword list.

_Evidence: HLP (keyword list + `resetKeyword…` handler names); 2.00a4 (full reset grammar); SyncTERM (ripper.c:8381 `rv_reset`); corpus (NEWSPAPR.RIP `$RESET(OVERFLOW)$`)._
...
_Deferred-command sentinel_

**Format:** `$OFF$` **Syntax:** `$OFF$`

`$OFF$` is not a mode toggle but a sentinel value: sent as the text of a deferred query ([RIP_QUERY](5.5-file-transfer-and-queries.md#rip_query) modes 1/2) it _cancels_ the stored deferred command, and 2.00a4 uses it likewise to disable refresh expressions ([RIP_SET_REFRESH](5.0-host-commands.md#rip_set_refresh) with `$OFF$` - see [`$NOREFRESH$`](#norefresh)).
...
_Evidence: 2.00a4; HLP - every processor in this group is named in the RIPSCRIP.HLP string table (`tvarProcPALENTRY` and peers); the table save/restore families are confirmed by the DLL's backup-system error strings ("No slots available for PUSH" / "No stack to pop")._
...
3.0's DLL accepts only 8 bits per channel - "RGB color mode only supports 8-bit color currently". _Evidence: 2.00a4; HLP._
...
The demo corpus branches on `$COLORS$` constantly - `<<IF $COLORS$<"256">>BLUEBACK.FN<<ELSE>>BLUEFADE.FN<<ENDIF>>` - making it the standard color-capability adaptive test (15 uses); see [5.2 Templates & Conditionals](5.2-templates-and-conditionals.md). _Evidence: 2.00a4; HLP; corpus (BUTTONS.RIP, MENU.RET)._
...
In 3.x, `$COMPAT$` is the single most common way scenes drop into legacy mode (21 corpus uses); SyncTERM additionally clears its coordinate remap tables. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8814-8840); corpus (BLUEBACK.FN)._
...
_Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8483/8530 save-restore groups; 9048 `rv_paste`)._
...
This is the standard "dismiss dialog, restore what was underneath" idiom; the demo entry/exit scripts comment it "Paste original screen image back". The RIPTEL.HLP command table lists the paste operation as "`$P$` paste clipboard" while the DLL string table names the processor `PCB` - both spellings likely resolve to the same handler, with `$P$` a shorthand _(hypothesis)_. _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:9048 `rv_paste`); corpus (MENU.ENT, TEL3X2.ENT)._
...
_Evidence: 2.00a4; HLP; SyncTERM (ripper.c:8719 `rv_mouse_kill`; 8483/8530 save-restore groups)._
...
SyncTERM backs the screen save/restore family with eleven screen slots (`screen_saves[11]`: 0-9 numbered plus a default). _Evidence: 2.00a4; HLP; SyncTERM (ripper.c:320, 8483 `rv_save`, 8530 `rv_restore`)._
...
> **Divergence:** SyncTERM's built-in table contains **`SAVE0` but no `RESTORE0`** (`RESTORE1`-`RESTORE9` only), whereas the HLP documents the symmetric `$RESTORE0$`-`$RESTORE9$` range - scripts should prefer the parameterized `$RESTORE(0)$` form. _(HLP; SyncTERM (ripper.c:8483, 8530))_
...
The 2.00a4 table-management family survives in the 3.0 DLL - every processor is named in the RIPSCRIP.HLP string table (`tvarProcCUR`, `tvarProcCOPY`, `tvarProcBACKSTAT`, and a `PROTECT` processor for `$PROT$`/`$UNPROT$`). _Evidence: 2.00a4; HLP (tvarProc string table)._
...
**3.x extension:** the corpus tests _text variables_ with a first argument of `TV` - `<<IF $INUSE(TV,NEXT_IMG)$="0">>$-=NEXT_IMG=ASTRO.JPG$<<ENDIF>>` (first-run initialization of the photo-cycling demo). _Evidence: 2.00a4; HLP; corpus (SPECLEFX.RIP, N2_PHOTO.RIP)._
...
## Image Placement Text Variables

_New in 3.x._

### $IMGSTYLE$

_Set destination port and rectangle for subsequent images_

**Format:** `$IMGSTYLE(cur,x0,y0,x1,y1)$`

Sets the image style - destination port and bounding rectangle - for subsequently displayed images and bitmaps, including those shown via the `$<file$` / `$(file$` [playback prefixes](5.3-local-playback.md). The first parameter names the target [drawing port](7.0-drawing-ports.md); the HLP documents `cur` as "the current screen port". This is the macro-language counterpart of the RIP_IMAGE_STYLE wire command (see [4.1 JPEG Images](4.1-jpeg-images.md)).
...
## Column Text System Paging

_New in 3.x._

### $OVERFLOW$

_Navigate column-system overflow pages_

**Format:** `$OVERFLOW(stream, cur|next|prev|page [,setverbose])$`

New in 3.x, and paired with the [column text system](3.1-column-text-system.md): flowed text that exceeds its columns paginates into numbered _overflow buffers_, and `$OVERFLOW$` navigates them. `page` yields the current page number ("Page `$overflow(1,page)$`" counters); `cur`/`next`/`prev` return the overflow content or file reference for use as a read-scene target - the demo pagers re-invoke their own scene: `ID=6:$overflow(1,prev,setverbose)$$>demo-01.col$`. [`$RESET(OVERFLOW)$`](#reset) deletes all overflow buffers.

_Evidence: corpus (DEMO-01.COL, DEMO-02.COL, NEWSPAPR.RIP); observed lowercase and uppercase spellings._

## File-Playback Prefixes

The four playback prefixes - `$>file.RIP$` (scene), `$)file.WAV$` (audio), `$<file.BMP$` (bitmap), `$(file.JPG$` (JPEG) - are syntactically text variables but form their own subsystem; they are documented with search-path and image-style rules in [5.3 Local File Playback](5.3-local-playback.md). The corpus uses `$>…$` 34 distinct ways (scene chaining is how the entire demo menu system navigates).

_Evidence: HLP; corpus; SyncTERM (ripper.c:7967-7970 implements `$>…$` only)._

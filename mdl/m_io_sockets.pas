...
Begin
  If FInBufPos = FInBufEnd Then Begin
    ...
    // A39: cache SocketError once per iteration - it's a function call (errno/
    // WSAGetLastError) that can change between checks if called repeatedly.
    While (FInBufEnd = -1) and (Error = ESOCKEWOULDBLOCK) Do Begin
      WaitMS(25);
...
Var
  ...
  Opt : LongInt;
Begin
  // IPv4 (PF_INET) bind.  A pure IPv6 (AF_INET6) socket fails to bind on
  // IPv4-only systems (e.g. Windows XP with no IPv6 stack), which stopped MIS
  // from listening.  NetInterface/Port come from the config (inetInterface).
  FSocketHandle := fpSocket(PF_INET, SOCK_STREAM, 0);

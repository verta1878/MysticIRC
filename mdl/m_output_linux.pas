...
// A39: get terminal size via the standard TIOCGWINSZ ioctl (robust) instead
// of parsing an ESC[18t reply (fragile - timing/terminal-dependent).
Procedure TOutputLinux.GetOriginalTermSize;
Var
  WinInfo : TWinSize;
Begin
  If fpIOCTL(1, TIOCGWINSZ, @WinInfo) >= 0 Then Begin
    TermXSize := WinInfo.ws_Col;
    TermYSize := WinInfo.ws_Row;
  End Else Begin
    TermXSize := 80;
    TermYSize := 25;
  End;
...
// Old escape-sequence version, kept for reference (replaced by the ioctl above):
(*
Procedure TOutputLinux.GetOriginalTermSize;
Var
  Str   : String;

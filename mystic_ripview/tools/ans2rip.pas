...
// ====================================================================
//
// ans2rip — ANSI/CHR to RIPscrip converter
//
// ====================================================================
// HOW IT WORKS:
//
...
Const
  ...
  { ANSI SGR color index → EGA/BIOS color index.
    SGR: 0=black 1=red 2=green 3=yellow 4=blue 5=magenta 6=cyan 7=white
    EGA: 0=black 1=blue 2=green 3=cyan   4=red  5=magenta 6=brown 7=lgray }
...
Begin
  ...
  // --- Pass 2: Foreground ---
  // PixelMode (-p): scan every character glyph, emit pixel-level bars.
  // Text mode: emit |@ text commands (smaller output, font-dependent).
  For Row := 0 to Rows - 1 Do Begin
    Col := 0;
    While Col < Cols Do Begin
      ...
      If PixelMode Or (Ch In [92, 124]) Then Begin
        // Pixel-perfect: scanline bars for this character
        PixX := Col * CHARW;
        PixY := Row * CHARH;
      ...
      // Text mode: collect run of same-fg chars, emit |@ command
      RunStart := Col;
      TextBuf := '';
      While (Col < Cols) And
  ...
  If PixelMode Then WriteLn('Mode: pixel-perfect')
  Else WriteLn('Mode: text (use -p for pixel-perfect)');
...
Begin
  ...
  If ParamCount < 2 Then Begin
    ...
    WriteLn;
    WriteLn('Usage: ans2rip [-p] input.ans output.rip');
    WriteLn;
    WriteLn('  -p  Pixel-perfect mode (scanline bars for every char)');
    WriteLn('      Without -p, uses RIP text commands (smaller output)');
    WriteLn;

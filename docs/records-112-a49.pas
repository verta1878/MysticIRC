// =========================================================================
// Mystic BBS Software                Copyright (C) 1997-2023 By James Coyle
// =========================================================================
// File    | RECORDS.PAS
...
Const
  ...
  mysCopyYear      = '1997-2023';
  mysVersion       = '1.12 A49';
  mysDataChanged   = '1.12 A49';
  mysServerStatVer = 0;
  ...
  {$IFDEF NODATE}
  mysOriginLine  = '--- ' + mysSoftwareID + ' v' + mysVersion + ' (' + OSID + OSBIT + ')';
  mysOriginNNTP  = '--- ' + mysSoftwareID + '/NNTP v' + mysVersion + ' (' + OSID + OSBIT + ')';
  mysOriginQWK   = '--- ' + mysSoftwareID + '/QWK v' + mysVersion + ' (' + OSID + OSBIT + ')';
  mysOriginQGate = '--- ' + mysSoftwareID + '/QWK Gate v' + mysVersion + ' (' + OSID + OSBIT + ')';
  {$ELSE}
  mysOriginLine  = '--- ' + mysSoftwareID + ' v' + mysVersion + ' ' + {$I %DATE%} + ' (' + OSID + OSBIT + ')';
  mysOriginNNTP  = '--- ' + mysSoftwareID + '/NNTP v' + mysVersion + ' ' + {$I %DATE%} + ' (' + OSID + OSBIT + ')';
  mysOriginQWK   = '--- ' + mysSoftwareID + '/QWK v' + mysVersion + ' ' + {$I %DATE%} + ' (' + OSID + OSBIT + ')';
  mysOriginQGate = '--- ' + mysSoftwareID + '/QWK Gate v' + mysVersion + ' ' + {$I %DATE%} + ' (' + OSID + OSBIT + ')';
  {$ENDIF}
...
Type
  ...
  AFDescType     = Array[1..26] of String[25];

  RecLogRoller = Record
    Level    : Byte;                   // 1=Normal, 2=Verbose, 3=DEBUG
    RollType : Byte;                   // 0=None, 1=File Size, 2=Day
    Files    : Word;                   // # Files to keep
    Size     : Cardinal;               // File size to trigger roll (KB)
    Cached   : Boolean;                // Cached log writing?
    DateFmt  : String[25];             //
  End;
...
Type
  RecEchoMailNode = Record
    ...
    AFlags      : AccessFlagType;
    SysopName   : String[30];
    UseSSL      : Byte;              //0=No, 1=Direct SSL, 2=StartTLS, 3=StarTLS/Forced
    CryptKey    : String[20];
    FileBoxHold : LongInt;
    CrashErrors : Word;
    Flags       : Byte;
    BinkPEscape : Byte;
    BinkULLimit : Cardinal;
    BinkDLLimit : Cardinal;
  ...
  RecSauceInfo = Record          // Used internally by Mystic
    Title     : String[35];
    Author    : String[20];
    Group     : String[20];
    FontName  : String[22];
    FileType  : Byte;
    DataType  : Byte;
    ColorMode : Byte;       // 0 = Unknown, 1 = Blink, 2 = iCE
    Columns   : Word;
    Lines     : Word;
  End;
 ...
 // INTERNALS
    ...
    RESERVED        : LongInt;                                  // permanent user # position
 // SYSTEM PATHS
    ...
    Python2Lib      : String[mysMaxPathSize];
    Python3Lib      : String[mysMaxPathSize];
 ...
 // GENERAL SETTINGS
    ...
    DefTermMode     : Byte;                                     // 0=ask 1=detect 2=detect/ask 3=ansi 4=ascii
    DefScreenSize   : Byte;
    DefScreenCols   : Byte;
 ...
 // LOGIN/MATRIX
    ...
    MatrixLogin     : Boolean;
    StartCodePage   : Byte;
    AutoWhiteIP     : Byte;                      // 0=Per User, 1=All
 // CONSOLE SETTINGS
 ...
 // NEW USER SETTINGS 1
    ...
    DefMsgPointers  : Byte;                                     // 0=First message, 1=EOM, 2=Last X, 3=Days Ago
    DefPointerData  : Word;                                     // Last messages or days ago
 // NEW USER SETTINGS 2
    ...
    AskCodePage     : Byte;
    UserTagLines    : Byte;                                     // 0=on 1=off 2=global 3=personal
 // NEW USER OPTIONAL
 ...
 // MESSAGE BASE SETTINGS
    ...
    AskMsgPost      : Byte;                      // 0=Yes, 1=No
 // ECHOMAIL NETWORKS
    NetAddress      : Array[1..mysMaxNetAddress] of RecEchoMailAddr;          // echomail addresses
    NetPrimary      : Array[1..mysMaxNetAddress] of Boolean;
    NetDomain       : Array[1..mysMaxNetAddress] of String[8];                // echomail domains (5D)
    NetDesc         : Array[1..mysMaxNetAddress] of String[25];               // echomail network description
 // LOGGING OPTIONS
    LogMIS          : RecLogRoller;
    LogNode         : RecLogRoller;
    LogPOLL         : RecLogRoller;
    LogQWK          : RecLogRoller;
 // OFFLINE MAIL
    QwkMaxBase      : Word;
    QwkMaxPacket    : Word;
 ...
 // UNSORTED/OTHER
    NoTimePerDay    : Boolean;
    INetMaxPollUL   : LongInt;
    INetMaxPollDL   : LongInt;
    AFDesc1         : AFDescType;
    AFDesc2         : AFDescType;
    EchoAFDesc      : AFDescType;
    UNUSED          : Array[1..399] of Char;
  End;
...
//  UserNoDelete   = $00000200;
  ...
  UserWhiteIP    = $00000800;
  UserInvisible  = $00001000;
...
Type
  ...
  RecUser = Record                                         // USERS.DAT
    ...
    QwkFiles     : Boolean;
    DateType     : Byte;
    TagLineMode  : Byte;                                   // 0=Ask,1=Off,2=AutoGlobal,3=AutoPersonal
    TagLineState : Byte;                                   // 0=Global, 1=Personal
    PeerIP       : String[45];
    PeerHost     : String[80];
...
Type
  RecMessageBase = Record                                  // MBASES.DAT
    ...
    LTemplate : String[20];                                     // lightbar list template
...
Const
  FDirOffline = $00000001;
  FDirUnvalid = $00000002;
  FDirDeleted = $00000004;
  FDirFailed  = $00000008;
  FDirFree    = $00000010;
  FDirHatched = $00000020;
...
Type
  ...
  RecMenuMarquee = Record
    Active : Boolean;
    X      : Word;
    Y      : Word;
    Field  : Word;
    Text   : Array[1..255] of Record
               C : Char;
               A : Byte;
             End;
    Offset : Word;
  ...
  RecMenuInfo = Packed Record
    ...
    DispFile    : String[20];
    Fallback    : String[20];
    NodeStatus  : String[40];
    Header      : String[250];
    Footer      : String[250];
    Marquee     : AnsiString;
    DoneX       : Byte;
    DoneY       : Byte;
    MenuType    : Byte;  // Menu: 0=std, 1=lb, 2=grid, 3=lbprompt
    InputType   : Byte;  // Hotkeys: 0=user, 1=no, 2=yes
    CharType    : Byte;
    DispCols    : Byte;
    Global      : Boolean;
    IdxCounter  : LongInt;
...
// CALLERS.DAT holds information on the last callers to the BBS up to 99
// records.
...
Type
  RecLastOn = Record                                            // CALLERS.DAT
    ...
    UserEmail  : Word;
    UserDoors  : Word;
    UserAF1    : AccessFlagType; // 4 bytes
    UserAF2    : AccessFlagType;
  ...
  ServerInfoRecord = Record
    ...
    SSLPort        : Word;
    AutoWhiteList  : Byte;
    POP3ForceSSL   : Boolean;
    BINKP_Escape   : Byte;
    BINKP_StartTLS : Byte;                       // 0=No, 1=Yes, 2=Forced
    DownloadLimit  : LongInt;
    UploadLimit    : LongInt;
    FTPTestUpload  : Boolean;
    FTPTestPass    : Byte;
    FTPTestCmdLine : String[80];
    Reserved       : Array[1..109] of Byte;
  End;

Const
  afAnnounced = $00000001;

Type
  AnnounceRecord = Record                        // ANNOUNCE.DIR & DES
    Flags     : LongInt;
    DateTime  : LongInt;
    EchoTag   : String[40];
    FileName  : String[70];

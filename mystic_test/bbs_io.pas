...
Type
  ...
  TBBSIO = Class
    ...
    NoFile         : Boolean;
    Graphics       : Byte;
    UseRipTerm     : Boolean;     { RIPscrip v1.54 mode active }
...
Begin
  ...
  Case Code[1] of
    ...
    'T' : Case Code[2] of
            ...
            'E' : Case Graphics Of
                    TERM_ASCII : LastMCIValue := 'Ascii';
                    TERM_ANSI  : LastMCIValue := 'Ansi';
                    TERM_RIP   : LastMCIValue := 'RIP';
                  Else
                    LastMCIValue := 'Unknown';
                  End;
...
Var
  ...
  Function GetParam (SepChar: Char) : String;
  Begin
    Result := '';

    Inc (Count);

    While (Str[Count] <> SepChar) and (Count <= Length(Str)) Do Begin
...
Begin
  ...
  While Count <= Length(Str) Do Begin
    If (Str[Count] = '|') and (Count < Length(Str) - 1) Then Begin
      ...
      If Not ParseMCI (True, Copy(Str, Count + 1, 2)) Then Begin
        BufAddChar(Str[Count]);
        Inc(Count);
      ...
      If FmtString Then Begin
        ...
        If FmtType in [18..19] Then Begin
          FmtString := False;
          Param1    := GetParam('#');
          Param2    := GetParam('#');
          Count     := Count + 1;
        ...
        FmtLen := strS2I(Copy(Str, Count + 1, 2));
        Inc (Count, 2);
        ...
        Case FmtType of
          4 : Begin
                Inc (Count);
                FmtString := False;
                BufAddStr (strRep(Str[Count], FmtLen));
              End;
          6 : Begin
                AnsiMoveX (FmtLen);
                FmtString := False;
          ...
          17: Begin
                ...
                If Console.CursorX < FmtLen Then
                  BufAddStr (strRep(Str[Count], FmtLen - Console.CursorX + 1));
              End;
        End;
      End;
    End Else
...
Var
  ...
  Begin
    ...
    { RIP terminal: try .mrp (menu) first, then .rip (art), then .ans }
    If (Graphics = TERM_RIP) and (FileExist(Temp + '.mrp')) Then Begin
      Ext    := '.mrp';
      FName  := Temp;
      Result := True;
    End Else
    If (Graphics = TERM_RIP) and (FileExist(Temp + '.rip')) Then Begin
      Ext    := '.rip';
      FName  := Temp;
      Result := True;
    End Else
    If (Graphics >= TERM_ANSI) and (FileExist(Temp + '.ans')) Then Begin
      Ext    := '.ans';
      FName  := Temp;
      Result := True;
    End Else

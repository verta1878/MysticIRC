...
Procedure SetThemeBoxDefaults (Var Theme: RecTheme);
Begin
  With Theme do Begin
    BoxShadow     := True;
    BoxShadowAttr := 8;
...
Procedure EditMessageBox (Var Theme: RecTheme);
Var
  Box   : TAnsiMenuBox;
  Form  : TAnsiMenuForm;
  Topic : String;
Begin
  ...
  Box.Header := ' Message Box ';

  Box.Open     (23, 5, 57, 20);
  VerticalLine (40, 7, 16);

  Form.AddTog  ('F', ' Frame Type'    , 28,  7, 42,  7, 12, 13, 1, 8, 'Single Double Single/Double Double/Single Block1 Block2 None Dotted', @Theme.BoxFrame, Topic + 'Border frame type for box');
  Form.AddAttr ('1', ' Frame Color 1' , 25,  8, 42,  8, 15, @Theme.BoxAttr, Topic + 'Color of top and right border and box background');
  Form.AddAttr ('2', ' Frame Color 2' , 25,  9, 42,  9, 15, @Theme.BoxAttr2, Topic + 'Color of left and bottom border');
  Form.AddAttr ('3', ' Frame Color 3' , 25, 10, 42, 10, 15, @Theme.BoxAttr3, Topic + 'Color of bottom left corner');
  Form.AddAttr ('4', ' Frame Color 4' , 25, 11, 42, 11, 15, @Theme.BoxAttr4, Topic + 'Color of top right corner');
  Form.AddBol  ('U', ' Use Shadow'    , 28, 12, 42, 12, 12, 3, @Theme.BoxShadow, Topic + 'Use shadow effect?');
  Form.AddAttr ('W', ' Shadow Color'  , 26, 13, 42, 13, 14, @Theme.BoxShadowAttr, Topic + 'Color of shadow');
  Form.AddAttr ('H', ' Header Color'  , 26, 14, 42, 14, 14, @Theme.BoxHeadAttr, Topic + 'Color of header text');
  Form.AddAttr ('T', ' Text Color'    , 28, 15, 42, 15, 12, @Theme.BoxTextAttr, Topic + 'Color of msg text');
  Form.AddAttr ('C', ' Confirm Color' , 25, 16, 42, 16, 15, @Theme.BoxOKAttr, Topic + 'Color of OK button');

  Form.AddNone ('S', ' SHOW EXAMPLE'  , 25, 18, 25, 18, 14, Topic + 'Show box using theme settings');
  Form.AddNone ('D', ' DEFAULTS'      , 46, 18, 46, 18, 10, Topic + 'Reset to defaults');

  Repeat
    Case Form.Execute of
      'S' : ThemeMessageBox(Theme, 0, 'Header', 'Example message text');
      'D' : If ShowMsgBox (1, 'Reset defaults?') Then
              SetThemeBoxDefaults(Theme);
      #27 : Break;
    End;
  Until False;
...
Begin
  ...
  // Icon Path removed â€” RIP editor moved to mystic_test/
  // Font Path removed â€” RIP editor moved to mystic_test/
  ...
  Form.AddBits ('R', ' Allow RIP'      , 11, 16, 25, 16, 12, ThmAllowRIP,  @Theme.Flags, Topic + 'Allow RIP users to use this theme?');
  ...
  Form.AddNone ('4', ' 4: Message Box' , 57, 10, 57, 10, 17, Topic + 'Edit message box style used in this theme');
      ...
      '/' : Case GetCommandOption(9, 'I-Insert|D-Delete|C-Copy|P-Paste|') of
              'I' : Begin
                      ...
                      With Theme Do Begin
                        ...
                        // IconPath removed -- RIP moved to mystic_test/
                        // FontPath removed -- RIP moved to mystic_test/

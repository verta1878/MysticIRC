...
Begin
  ...
  Console.WriteXY (14, 11, 113, 'Copyright (C) 1997-2013 James Coyle');
  Console.WriteXY (14, 12, 113, 'IRC Fork (C) 2025-2026 verta1878, sysop/0, evga, kiddo, wrench');
...
// A3: MPL Help Index â€” searchable function reference
...
Begin
  ...
  If IOResult = 0 Then Begin
    While Not Eof(TF) Do Begin
      ...
      Else If (Length(Line) > 4) and (Line[1] = ' ') and (Line[2] = ' ') and (Line[3] = ' ') and (Line[4] = ' ') Then
        // Description line â€” append to previous
  ...
  If SearchWord <> '' Then Begin
    ...
    Found := False;
    List.Picked := 1;
    // Simple: scan list items for match â€” TMenuList.Picked is 1-based
...
//                #64 : SearchText(True);
  ...
  Console.WriteLine('Copyright (C) 1997-2013 James Coyle. IRC Fork (C) 2025-2026');

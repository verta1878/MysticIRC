...
Type
  TMsgBase = Class
    ...
    Procedure   AreaIndex           (CmdData: String);
...
Begin
  ...
  If IoResult = 0 Then Begin
    ...
    While Not MsgBase^.EOM Do Begin
      ...
      // A2-02: Strip ANSI escape sequences from quoted text
      // so color codes don't bleed into the quote
      While Pos(#27 + '[', TempStr) > 0 Do Begin
        A := Pos(#27 + '[', TempStr);
        B := A + 2;
        While (B <= Length(TempStr)) and Not (TempStr[B] in ['A'..'Z', 'a'..'z']) Do
          Inc(B);
        If B <= Length(TempStr) Then
          Delete(TempStr, A, B - A + 1)
        Else
          Delete(TempStr, A, Length(TempStr) - A + 1);
      End;
...
Procedure AreaIndexSearch (Var Owner: Pointer; Str: String);
Begin
  If Str = '' Then
    Session.io.OutFull(Session.Template.Prompt[5])
  Else Begin
    If Length(Str) = 1 Then
      Session.io.OutFull(Session.Template.Prompt[6]);

    If Length(Str) > 15 Then
      Str := Copy(Str, Length(Str) - 15 + 1, 255);
  End;

  WriteXY (Session.Template.Screen[3].X, Session.Template.Screen[3].Y, Session.Template.Screen[3].A, strPadR(Str, 15, ' '));
End;

Procedure AreaIndexDrawBar (X, Y, W: Byte; IsHi: Boolean; BarName: String; Tagged: Byte);
Var
  Str : String;
Begin
  If BarName = '' Then Exit;

  Session.io.PromptInfo[1] := strWordGet(1, BarName, #0);
  Session.io.PromptInfo[2] := strWordGet(2, BarName, #0);
  Session.io.PromptInfo[3] := strWordGet(3, BarName, #0);
  Session.io.PromptInfo[4] := strWordGet(4, BarName, #0);
  Session.io.PromptInfo[5] := strWordGet(5, BarName, #0);

  If BarName[1] = #2 Then Begin
    Delete (Session.io.PromptInfo[1], 1, 1);

    Str := Session.Template.Prompt[7];
  End Else Begin
    If IsHi Then
      If Session.io.PromptInfo[4] <> '0' Then
        Str := Session.Template.Prompt[4]
      Else
        Str := Session.Template.Prompt[3]
    Else
    If Session.io.PromptInfo[4] <> '0' Then
      Str := Session.Template.Prompt[2]
    Else
      Str := Session.Template.Prompt[1];
  End;
...
// Post message (tag bases?)
// Catch up (tag bases?)
// Help
// Toggle newscan / newscan in prompts?
// include email base?
// only show new bases?  only show newscan toggled bases?
// ignore groups / honor groups
// recalculate/sort stats
// goto/select next base with new msgs
// prompt for "non-highlighted tagged base"
// prompt for "highlighted tagged base"
// find/text search messages (text highlighting?)
//    always use index here?
//    tagged bases

Procedure TMsgBase.AreaIndex (CmdData: String);
Var
  OldGroup      : RecGroup;
  ...
  Procedure AddSort (Col: Byte; Mode: TAnsiListBoxSort);
  Begin
    SortPos := SortPos + 1;

    ListBox.SortDepth[SortPos].Column := Col;
    ListBox.SortDepth[SortPos].Mode   := Mode;
  End;
  ...
  Begin
    ...
    If ShowDivisions Then AddSort(8, sortAscending);
    If ShowNewAtTop  Then AddSort(7, sortDecending);

    AddSort (1, sortAscending);

    BaseFile := TFileBuffer.Create(16 * 1024);

    If Not BaseFile.OpenStream(bbsCfg.DataPath + 'mbases.dat', SizeOf(TempBase), fmOpen, fmRWDN) Then Begin
      BaseFile.Free;
    ...
    BaseFile.ReadRecord(TempBase);

    While Not BaseFile.EOF Do Begin
      BaseFile.ReadRecord(TempBase);
    ...
    If ShowDivisions Then Begin
      ...
      For Count := 1 to ListBox.ListMax Do Begin
        CurDiv := strS2I(strWordGet(8, ListBox.List[Count]^.Name, #0));
...
Var
  ...
  Found     : Boolean;
Begin
  If Session.io.Graphics = 0 Then Exit;

  OldGroup  := Group;
  OldMBase  := MBase;

  SaveGroup := Session.User.IgnoreGroup;
  ListBox   := TAnsiListBox.Create;

  Template := Session.ReadTemplate ('ansimidx', False);

  ShowDivisions := Template.ReadBoolean(TemplateOptions, 'show_divs', True);
  ...
  ListBox.NoWindow   := True;
  ListBox.LoChars    := #13#14#18#26#27;
  ListBox.PercentBar := Session.Template.PercentBar;

  ListBox.SetSearchProc(@AreaIndexSearch);
  ListBox.SetUpdateProc(@AreaIndexDrawBar);
  ...
  Repeat
    ...
    If ReDraw Then Begin
      Session.io.OutFile (CmdData, False, 0);

      If SnapToNew Then
        For Count := 1 to ListBox.ListMax Do
          If strS2I(strWordGet(4, ListBox.List[Count]^.Name, #0)) > 0 Then Begin
            ListBox.Picked := Count;
    ...
    ListBox.Open (Session.Template.Screen[1].X-1, Session.Template.Screen[1].Y-1, Session.Template.Screen[2].X+1, Session.Template.Screen[2].Y+1);

    Case ListBox.ExitCode of
      #13 : Begin
              If GetMBaseByIndex(strS2I(strWordGet(6, ListBox.List[ListBox.Picked]^.Name, #0)), TempBase) Then Begin
                ...
                Else
                  ReadMessages('L', TempStr, '');

                GetMessageStats (False, False, False, Count, TempBase, False, False, MsgTotal, MsgNew, MsgYours);

                ListBox.List[ListBox.Picked]^.Name :=
                  TempBase.Name + #0 + bbsCfg.NetDesc[TempBase.NetAddr] + #0 + strComma(MsgTotal) + #0 + strComma(MsgNew) + #0 + strComma(MsgYours) + #0 + strI2S(TempBase.Index) + #0 + strI2S(Ord(MsgNew > 0)) + #0 + strI2S(TempBase.NetAddr);
      ...
      #14 : Begin
              ...
              For Count := ListBox.Picked + 1 to ListBox.ListMax Do
                If strS2I(strWordGet(4, ListBox.List[Count]^.Name, #0)) > 0 Then Begin
                  ListBox.Picked := Count;
                  Found          := True;
              ...
              If Not Found Then
                For Count := 1 to ListBox.Picked Do
                  If strS2I(strWordGet(4, ListBox.List[Count]^.Name, #0)) > 0 Then Begin
                    ListBox.Picked := Count;
...
Begin
  If (MBase.NetType > 0) and (MBase.QwkNetID = 0) Then Begin
    ...
    // A2-01: Add TZUTC kludge per FTS-4008
    // Format: HHMM, no '+' for east, '-' prefix for west
    Begin
      TZOfs := GetLocalTimeOffset;  // minutes, negative = west
      If TZOfs < 0 Then
        TZStr := '-' + strPadL(strI2S(Abs(TZOfs) DIV 60 * 100 + Abs(TZOfs) MOD 60), 4, '0')
      Else
        TZStr := strPadL(strI2S(TZOfs DIV 60 * 100 + TZOfs MOD 60), 4, '0');
      Msg^.DoStringLn (#1 + 'TZUTC: ' + TZStr);
    End;
    ...
    Case TempBase.NetType of
      1 : Begin
            If TempBase.QwkConfID = 0 Then
              Session.SemEchomail := True
            Else
              Session.SemQwkNet := True;
          End;
      2 : Session.SemUseNet  := True;
      3 : Session.SemNetmail := True;
    End;

    If bbsCfg.CreateSemaphore = 1 Then
      Session.WriteSemFiles;
...
Var
  ...
  Begin
    ...
    While Not MsgBase^.EOM and (Lines < mysMaxMsgLines) Do Begin
      ...
      // A4-03: Strip kludge lines, tear line, and origin when editing
      // These are regenerated by AppendMessageText on save
      If (MsgText[Lines][1] = #1) Then Begin
        Dec(Lines);
        Continue;
      End;
      If Copy(MsgText[Lines], 1, 4) = '--- ' Then Begin
        // Tear line and everything after (origin) is stripped
        Dec(Lines);
        // Skip remaining lines (origin, seen-by, path)
        While Not MsgBase^.EOM Do
          MsgBase^.GetString(79);
        Break;
      End;
    End;

    // Trim trailing blank lines
    While (Lines > 1) and (MsgText[Lines] = '') Do
      Dec(Lines);
    ...
    Repeat
      ...
      Case Session.io.OneKeyRange('Q?', 1, Total) of
        #00: If GetBaseCompressed(Session.io.RangeValue, TempBase) Then Begin
               ...
               // A5-01: Strip network information when forwarding
               // Kludges, tear, origin, seen-by, path are stripped
               // and recalculated for the destination base
               While Not MsgBase^.EOM Do Begin
                 Str := MsgBase^.GetString(79);

                 // Skip kludge lines
                 If (Length(Str) > 0) and (Str[1] = #1) Then Continue;
                 // Skip tear line and everything after (origin, seen-by, path)
                 If Copy(Str, 1, 4) = '--- ' Then Break;
                 // Skip SEEN-BY and PATH (can appear without tear)
                 If Copy(Str, 1, 9) = 'SEEN-BY: ' Then Continue;
                 If (Length(Str) > 0) and (Str[1] = #1) Then Continue;

                 MsgNew^.DoStringLn(Str);
               End;

               // Regenerate network info for destination base
               If TempBase.NetType > 0 Then Begin
                 MsgNew^.DoStringLn(#13 + '--- ' + mysSoftwareID + ' v' + mysVersion + ' (' + OSID + ')');

                 Str := ' * Origin: ' + ResolveOrigin(TempBase);
                 If TempBase.QwkNetID = 0 Then
                   Str := Str + ' (' + Addr2Str(bbsCfg.NetAddress[TempBase.NetAddr]) + ')';
                 MsgNew^.DoStringLn(Str);
               End;
...
*)
  ...
  Var
    HasAnsi : Boolean;
  Begin
    // A1-11: Detect embedded ANSI sequences in message text
    HasAnsi := Pos(#27 + '[', Str) > 0;

    If HasAnsi Then Begin
      // Pass ANSI directly — don't override with message base colors

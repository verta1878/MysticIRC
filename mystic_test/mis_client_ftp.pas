...
Uses
  DOS,
//  SysUtils,  //for wordrec only? (replaced by MyWordRec below)
  m_io_Base,
  m_io_Sockets,
  m_Strings,
  m_FileIO,
...
Begin
  If LoggedIn Then Begin
    ...
    MyWordRec(DataPort).Hi := strS2I(Copy(Data, 1, Pos(',', Data) - 1));
    MyWordRec(DataPort).Lo := strS2I(Copy(Data, Pos(',', Data) + 1, Length(Data)));
...
Begin
  If LoggedIn Then Begin
    ...
    {$IFDEF FTPDEBUG}
      ...
      Server.Status(ProcessID, re_PassiveOK + '(' + strReplace(Client.HostIP, '.', ',') + ',' + strI2S(MyWordRec(DataPort).Hi) + ',' + strI2S(MyWordRec(DataPort).Lo) + ').');
    {$ENDIF}

    Client.WriteLine(re_PassiveOK + '(' + strReplace(Client.HostIP, '.', ',') + ',' + strI2S(MyWordRec(DataPort).Hi) + ',' + strI2S(MyWordRec(DataPort).Lo) + ').');

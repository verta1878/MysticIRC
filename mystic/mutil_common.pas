...
Var
  ...
  LogFile      : String;
  LogLevel     : Byte = 1;
  LogStamp     : String  = 'NNN DD HH:II:SS';
  MaxLogFiles  : LongInt = 10;
...
Begin
  ...
  For I := MaxLogFiles - 1 DownTo 1 Do Begin
    A := LogFile + '.' + strI2S(I);
    B := LogFile + '.' + strI2S(I + 1);
    If FileExist(A) Then FileRename(A, B);
  End;

  FileRename(LogFile, LogFile + '.1');
End;
...
Begin
  ...
  If Not LogIsOpen Then Begin
    LogRoll;                       // A40.4: rotate oversized log at first write
    ...
    LogIsOpen := True;             // A40.5: keep open across writes
  End;

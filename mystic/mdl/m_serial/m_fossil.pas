...
Type
  ...
  Public
    ...
    Function  RecvByte(Var B: Byte): Boolean;  // function 02h: read one byte
    ...
    Procedure PurgeOutput;                  // function 09h
    Function  Status: Word;                 // function 03h (full status word)
    Procedure SetBaud(BaudInit: Byte);      // function 00h (baud init byte)
...
Function TFossil.Status: Word;
Begin
  Result := 0;
  If Not FActive Then Exit;
  Case FBackend of
    fbSerial : Begin
                 If FSer.DataAvailable Then Result := Result or $0100; { RX ready }
                 Result := Result or $2000; { TX ready — always for serial }
                 If FSer.GetDCD Then Result := Result or $0080; { DCD }
...
Procedure TFossil.SetBaud(BaudInit: Byte);
Begin
  If Not FActive Then Exit;
  Case FBackend of
...
Function TFossil.RecvByte(Var B: Byte): Boolean;
Var
  C: Char;
Begin
  Result := False;
  ...
  Case FBackend of
    fbSerial : Begin
                 If FSer.DataAvailable Then Begin
                   If FSer.ReadBuf(C, 1) = 1 Then Begin
                     B := Byte(C);
                     Result := True;
                   End;
                 End;
               End;
    fbInt14  : {$IFDEF FOSSIL_INT14}
               Begin
                 If (Int14($03, $00, FPort) and $0100) <> 0 Then Begin
                   B := Lo(Int14($02, $00, FPort));
                   Result := True;
                 End;
               End;
               {$ELSE} ;

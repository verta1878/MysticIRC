...
Const
  ...
  DATEC1970   = 2440588;                     // Julian day # of 1970-01-01 (Unix epoch) [A39]
...
Begin
  ...
  // A39: parse the time portion too, if the string carries one
  // (e.g. 'MM/DD/YY HH:II'); A38 always zeroed the time.
  If Length(Str) > 8 Then Begin
    DT.Hour := strS2I(Copy(Str, 10, 2));
    DT.Min  := strS2I(Copy(Str, 13, 2));
  End;
...
Begin
  ...
  DOWStr  := DayString[DayOfWeek(DosDate)];
  ...
  Result  := strReplace(Result, 'DDD', DOWStr);

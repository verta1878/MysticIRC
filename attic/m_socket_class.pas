...
// --------------------------------------------------------------------
// This unit is the legacy TSocketClass socket implementation.  It is NO
// LONGER USED by the live code: the active socket class is TIOSocket in
// mdl/m_io_sockets.pas (confirmed by the live Mystic 1.10 A51 binary,
// which contains TIOSocket and zero TSocketClass).  g00r00's own A39-era
// experiment; abandoned by A51.  Retained (not deleted) for GPL-3
// attribution and as part of Mystic's development history - and as a
// reference pure-IPv4 socket implementation should the XP/IPv6 question
// ever push toward an IPv4 rewrite of TIOSocket.
// Retired from mdl/ on 2026-07-06 (fork).  Its standalone test,
// mdltest5.pas, is retired alongside it.
// ====================================================================

// ====================================================================
// NOTE (fork): This unit (m_socket_class / TSocketClass) is LEGACY and is
// NO LONGER USED by the live code.  It is retained only for reference to
// the original source.  The only reference to it is mdl/mdltest5.pas (a
// standalone test program).  The live socket class is TIOSocket in
// mdl/m_io_sockets.pas, which is what MIS and the BBS actually use.
// This file may be deleted or moved outside the main src branch in future.
// ====================================================================
// ====================================================================
...
{$ENDIF}
{$IFDEF WINDOWS}
// Fork addition: native Win32 resolver (XP-safe, IPv4 via gethostbyname).
// Unix keeps the C glue (m_resolve_address.o). IPv6-on-Win32 = Phase 3 TODO
// (use WSAAddressToString; inet_ntop is Vista+ and absent on XP).
Function ResolveAddress_IPv6(Host:PChar; Remote_Address:PChar):Integer;
Var HE:PHostEnt; P:PChar; S:String; Q:String[3]; i:LongInt;
Begin
  Result := 0;
  HE := gethostbyname(Host + 1);
  If (HE=Nil) or (HE^.h_addr_list=Nil) or (HE^.h_addr_list^=Nil) Then Exit;
  P := HE^.h_addr_list^;              // 4 IPv4 bytes, network order
  S := '';
  For i := 0 To 3 Do Begin
    ...
    S := S + Q;
  End;
  (Remote_Address+0)^ := Char(Length(S));
  Move(S[1], (Remote_Address+1)^, Length(S));
  Result := AF_INET;
End;
{$ENDIF}

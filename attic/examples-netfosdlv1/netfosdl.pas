...
procedure Int14Handler(
  { Real-mode 16-bit interrupt frame. This parameter order (Flags first,
    then the pushed registers) is what FPC's i8086-msdos target and Turbo
    Pascal both expect. REQUIRES a real-mode compiler: FPC 3.0+ with
    -Ti8086 -Tmsdos (the fpc264irc ppcross8086), or Turbo Pascal 7. It will
    NOT build under a protected-mode DOS target (FPC go32v2), whose
    interrupt frame is 32-bit — that is by design, this is a real-mode TSR. }
...
begin
  ...
  asm
    ...
    push ax           { preserve top-of-stack across the DOS call }
    mov  ah, 62h      { INT 21h/62h: get PSP segment -> BX. Works on any }
    int  21h          {   real-mode target (Turbo Pascal, FPC 3.x i8086) }
    pop  ax           {   with no RTL PrefixSeg symbol dependency. }
...
begin
  ...
  { Free the TSR memory. INT 21h/Fn 49h releases the program's
    memory block. PSP segment fetched via INT 21h/62h (portable). }
